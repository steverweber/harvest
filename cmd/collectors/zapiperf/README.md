...
Note: The main difference between this collector and the Zapi collector is that ZapiPerf collects only the `perf` subfamily of the ZAPIs. Additionally, ZapiPerf always calculates final values from deltas of two subsequentive polls (therefore data is emitted only after the second poll).
...
It is highly recommended to create a read-only user on the ONTAP system dedicated to Harvest. See [documentation of the Zapi collector](../zapi/README.md) for guidance.
...
This configuration file contains a list of objects that should be collected and the filenames of their configuration (explained in the next section).
...
| `latency_io_reqd`  | int, optional  | threshold of IOPs to calculate latencies (latencies based on very few IOPs are unreliable | `100`  |
